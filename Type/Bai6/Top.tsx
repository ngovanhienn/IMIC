...
const Top = () => {
  ...
  // };
  const getListRate = () => {
    const Url = 'https://api.themoviedb.org/3/movie/top_rated?api_key=3a4afa2731fe15939b309743eee95891&language=en-US&page=1';
    return fetch(Url)
    .then((res) => res.json())
    .then(json => {
      return json as Root;
...
const MovieComponent: React.FC<Props> = props =>{
  ...
  return (
    <View>
      <Text style={{textAlign: 'center', fontSize:30, color:'#000', marginBottom:10}}>List Top Rated Movie</Text>
      <FlatList
